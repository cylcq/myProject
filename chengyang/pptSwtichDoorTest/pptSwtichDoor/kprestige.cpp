...
{
	if (frame > 0.2 && frame < 0.8)
	{
		int horizontalSpringCount = m_Cloth.getHorizontalSpringCount();
		int verticalSpringCount = m_Cloth.getVerticalSpringCount();
		m_Cloth.getMassPoint(verticalSpringCount * 0.65, horizontalSpringCount * (0.8 - frame)).setPullPointXYZ(false, true, true);
		m_Cloth.getMassPoint(verticalSpringCount * 0.65, horizontalSpringCount * 0.6).setPullPointXYZ(false, true, true);
		if (1 - frame > 0)
		{
			m_Cloth.getMassPoint(verticalSpringCount * 0.65, horizontalSpringCount * (1 - frame) - 1).setPullPointXYZ(false, false, false);
		}
	}
	if (frame < 0.1)
	{
