...
void cloth::drawTriangle(massPoint *p1, massPoint *p2, massPoint *p3, const QVector<QVector2D>& vec)
{
    //glColor3fv( (GLfloat*) &color );

    glNormal3fv((GLfloat *) &(p1->getNormal().normalized() ));
	glTexCoord2fv((GLfloat*)&vec[0]);
    glVertex3fv((GLfloat *) &(p1->getPos() ));

    glNormal3fv((GLfloat *) &(p2->getNormal().normalized() ));
	glTexCoord2fv((GLfloat*)&vec[1]);
    glVertex3fv((GLfloat *) &(p2->getPos() ));

    glNormal3fv((GLfloat *) &(p3->getNormal().normalized() ));
	glTexCoord2fv((GLfloat*)&vec[2]);
...
cloth::cloth(float width, float height)
{
    ...
    //getmassPoint(0,(horizontalSpringCount-1) / 2)->fixPosition();
    //getmassPoint(0,horizontalSpringCount-2)->fixPosition();
    //getmassPoint(0,horizontalSpringCount-3)->fixPosition();

	//getmassPoint((horizontalSpringCount-1) / ,5)->fixPosition();

	int halfHor = verticalSpringCount / 2;
	getmassPoint(verticalSpringCount * 0.35,0)->fixPosition();
	getmassPoint(verticalSpringCount * 0.65, 0)->fixPosition();

	getmassPoint(verticalSpringCount * 0.35,horizontalSpringCount * 0.5)->fixPosition();
	getmassPoint(verticalSpringCount * 0.65,horizontalSpringCount * 0.5)->fixPosition();

	/*int  kv= 0;
	for(; kv < horizontalSpringCount ;)  
	{
		getmassPoint(halfHor - 1,kv)->fixPosition();
		kv++;
	}*/
}
...
void cloth::drawCloth()
 {
	...
	QVector<QVector3D> texCoordinate;
    glBegin(GL_TRIANGLES);
		for(int x = 0; x<verticalSpringCount-1; x++)
		{
			for(int y=0; y<horizontalSpringCount-1; y++)
			{
				...
				float float_x = float(x) / verticalSpringCount;
				float float_y = float(horizontalSpringCount - y)/horizontalSpringCount;
				float float_xb = float(x + 1)/verticalSpringCount;
				float float_yb = float(horizontalSpringCount - y - 1)/horizontalSpringCount;

				//左上角
				QVector2D leftTop(float_x,float_y);
				//左下角
				QVector2D leftBottom(float_x,float_yb);
				//右上角
				QVector2D rightTop(float_xb,float_y);
				//右下角
				QVector2D rightBottom(float_xb,float_yb);
				...
				drawTriangle(getmassPoint(x,y + 1),getmassPoint(x,y),getmassPoint(x + 1,y),vec);//upper left triangle
				...
				drawTriangle(getmassPoint(x+1,y+1),getmassPoint(x,y + 1),getmassPoint(x + 1,y),vec);//lower right triangle
			}
		}
    glEnd();
}
...
void cloth::triWindForce(massPoint *p1,massPoint *p2,massPoint *p3, const QVector3D direction)
{
    ...
    QVector3D force = normal*(QVector3D::dotProduct(d,direction));//点积在物理上表示功
